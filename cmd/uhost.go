	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if global.json {
				...
				base.PrintTable(list, []string{"UHostName", "ResourceID", "Group", "ClassicNetwork", "Config", "Type", "CreationTime", "State"})
			}
		},
	}
...
func NewCmdUHostCreate() *cobra.Command {
	...
	async = flags.Bool("async", false, "Optional. Display information about the operation in progress, without waiting for the operation to complete.")
	...
	bindEipID = flags.String("bind-eip-id", "", "Optional. Bind eip to uhost")
...
func NewCmdUHostDelete() *cobra.Command {
	...
	req.EIPReleased = cmd.Flags().String("eip-released", "false", "false,Unbind EIP only; true, Unbind EIP and release it")
...
func NewCmdUHostResize() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if inst.State == "Running" {
				agreeClose, err := ux.Prompt("Resize uhost must be after stop it. Do you want to stop this host? (y/n):")
				if err != nil {
					base.Cxt.Println(err)
					return
				}
	...
	req.DiskSpace = cmd.Flags().Int("data-disk-size-gb", 0, "Data disk size,unit GB. Range[10,1000], SSD disk range[100,500]. Step 10")
	req.BootDiskSpace = cmd.Flags().Int("system-disk-size-gb", 0, "System disk size, unit GB. Range[20,100]. Step 10. System disk does not support shrinkage")

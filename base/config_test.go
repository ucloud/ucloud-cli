...
// Global.OAuthBaseURL（--oauth-base-url 透传）非空时，mergeConfigIns 必须覆盖
// ins.OAuthBaseURL，与 --base-url 的瞬时覆盖语义对称。
func TestMergeConfigInsOAuthBaseURLOverride(t *testing.T) {
	saved := Global.OAuthBaseURL
	defer func() { Global.OAuthBaseURL = saved }()

	Global.OAuthBaseURL = "https://oauth-test.example/"
	ins := &AggConfig{OAuthBaseURL: "https://stale.example/", MaxRetryTimes: intPtr(3)}
	mergeConfigIns(ins)
	if ins.OAuthBaseURL != "https://oauth-test.example/" {
		t.Errorf("expected global --oauth-base-url to override, got %q", ins.OAuthBaseURL)
	}
}

// config update --oauth-base-url 的落盘语义：把 oauth_base_url 应用到已存在 profile 后
// 经 UpdateAggConfig 落盘，重新读盘应能取回（镜像 configure.go 的 apply+persist 路径）。
func TestOAuthBaseURLUpdatePersist(t *testing.T) {
	os.MkdirAll(".ucloud", 0700)
	defer os.RemoveAll(".ucloud")
	m, err := NewAggConfigManager(".ucloud/config.json", ".ucloud/credential.json")
	if err != nil {
	...
	base := &AggConfig{
		Profile: "p", Active: true, BaseURL: DefaultBaseURL, Timeout: 15, MaxRetryTimes: intPtr(3),
	}
	if err := m.Append(base); err != nil {
		t.Fatal(err)
	}

	// 模拟 `config update --oauth-base-url ...`：取 cacheConfig，应用字段，落盘
	cacheConfig, ok := m.GetAggConfigByProfile("p")
	if !ok {
		t.Fatal("profile p missing")
	}
	cacheConfig.OAuthBaseURL = "https://oauth-upd.example/"
	if err := m.UpdateAggConfig(cacheConfig); err != nil {
		t.Fatal(err)
	}
	...
	got, ok := m2.GetAggConfigByProfile("p")
	if !ok {
		t.Fatal("profile p missing after reload")
	}
	if got.OAuthBaseURL != "https://oauth-upd.example/" {
		t.Errorf("oauth_base_url not persisted on update, got %q", got.OAuthBaseURL)
	}
}

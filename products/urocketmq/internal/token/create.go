...
// newCreate ucloud urocketmq token create
func newCreate(ctx *cli.Context) *cobra.Command {
	...
	req := client.NewCreateURocketMQTokenRequest()
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create URocketMQ token",
		Long:  "Create URocketMQ token",
		Run: func(cmd *cobra.Command, args []string) {
			...
			resp, err := client.CreateURocketMQToken(req)
			if err != nil {
				ctx.HandleError(err)
				return
			}
			ctx.EmitResult(cli.OpResultRow{ResourceID: resp.TokenId, Action: "create", Status: "Success"})
		},
	}

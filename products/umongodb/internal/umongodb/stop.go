...
// newStop implements `umongodb stop`.
func newStop(ctx *cli.Context) *cobra.Command {
	var async bool
	var ids []string

	client := cli.NewServiceClient(ctx, umongodb.NewClient)
	req := client.NewStopUMongoDBClusterRequest()

	cmd := &cobra.Command{
		Use:   "stop",
		Short: "Stop MongoDB instances",
		Long:  "Stop one or more running MongoDB instances.",
		Run: func(c *cobra.Command, args []string) {
			...
			for _, idName := range ids {
				...
				_, err := client.StopUMongoDBCluster(req)
				if err != nil {
					ctx.HandleError(err)
					continue
				}
				...
				if async {
					...
					ctx.PollerTo(w, describeByID(ctx, *req.Region, *req.Zone)).Spoll(id, text, []string{stateStopped, stateFail})
				}
	...
	flags.StringSliceVar(&ids, resourceIDFlag, nil, "Required. Cluster ID(s) of MongoDB instances to stop.")
	...
	command.SetCompletion(cmd, resourceIDFlag, func() []string {
		return getMongoDBIDList(ctx, []string{stateRunning}, *req.Region, *req.Zone, *req.ProjectId)
	})

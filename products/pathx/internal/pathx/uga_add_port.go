...
func newUGAAddPort(ctx *cli.Context) *cobra.Command {
	var ports []string
	var protocol string
	client := cli.NewServiceClient(ctx, ppathx.NewClient)
	req := client.NewAddUGATaskRequest()
	cmd := &cobra.Command{
		Use:   "add-port",
		...
		Run: func(c *cobra.Command, args []string) {
			...
			*req.UGAId = ctx.PickResourceID(*req.UGAId)
			_, err = client.AddUGATask(req)
			if err != nil {
				ctx.HandleError(err)
				return
			}
			fmt.Fprintf(ctx.ProgressWriter(), "port %v added\n", ports)
			ctx.EmitResult(cli.OpResultRow{ResourceID: *req.UGAId, Action: "add-port", Status: "Added"})
		},
	}
	flags := cmd.Flags()
